...
use crate::handlers_core::{
    HandlerError, admin_login_impl, change_password_impl, confirm_password_reset_impl,
    login_impl, logout_impl, protected_response, refresh_impl, register_impl,
    request_password_reset_impl,
};
use crate::models::{
...
impl HandlerError {
    fn into_http_response(self) -> HttpResponse {
        match self {
            ...
            Self::Conflict(message) => HttpResponse::Conflict().json(ApiResponse::<()>::error(message)),
            Self::NotFound(message) => HttpResponse::NotFound().json(ApiResponse::<()>::error(message)),
...
pub async fn logout(
    ...
    match logout_impl(data.get_ref(), &req).await {
        Ok(response) => HttpResponse::Ok().json(ApiResponse::success(response, "Logout successful")),
        Err(error) => error.into_http_response(),
    }
}
